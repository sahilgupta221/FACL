...
public class SimScenario implements Serializable {
	...
	private static final String APP_PACKAGE = "applications.";
        /*change number of guardians*/
        public static final String GUARD_NUMBER = "guardiannumber";        
	public int guardCount; /**change how many nodes in this formation */

        /** The world instance */
	private World world;
        protected List<DTNHost> guardian_list; //change       
	...
	protected SimScenario() {
		...
		this.eqHandler = new EventQueueHandler();
                this.guardian_list = new ArrayList<DTNHost>();
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			int appCount;
                        this.guardCount = s.getInt(GUARD_NUMBER);
			...
			if (mmProto instanceof MapBasedMovement) {
				this.simMap = ((MapBasedMovement)mmProto).getMap();
                        ...
                        int formationIndex=-1; //finding proper guardian
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				hosts.add(host);
                               // update guardian local group address id 
                               if(host.groupId.equals("Guardian"))
                               {
